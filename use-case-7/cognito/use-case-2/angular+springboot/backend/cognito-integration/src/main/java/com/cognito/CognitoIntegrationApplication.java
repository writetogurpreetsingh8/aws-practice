...
public class CognitoIntegrationApplication {
	...
	/*
	 * private static DecodedJWT verifyToken(String token) throws
	 * MalformedURLException, IllegalArgumentException, JwkException,
	 * JsonMappingException, JsonProcessingException { String issuer =
	 * "https://cognito-idp.us-east-1.amazonaws.com/us-east-1_QMlDKndwa";
	 * 
	 * URL jwksUrl = new URL(issuer + "/.well-known/jwks.json"); JwkProvider
	 * provider = new UrlJwkProvider(jwksUrl);
	 * 
	 * DecodedJWT jwt = JWT.decode(token); Jwk jwk = provider.get(jwt.getKeyId());
	 * 
	 * Algorithm algorithm = Algorithm.RSA256((RSAPublicKey) jwk.getPublicKey(),
	 * null); JWTVerifier verifier = JWT.require(algorithm) .withIssuer(issuer)
	 * .build();
	 * 
	 * DecodedJWT token1 = verifier.verify(token);
	 * System.out.println(token1.getPayload()); // Decode payload String
	 * base64Payload = token1.getPayload(); byte[] decodedBytes =
	 * Base64.getUrlDecoder().decode(base64Payload); String jsonPayload = new
	 * String(decodedBytes);
	 * 
	 * // Parse JSON payload ObjectMapper objectMapper = new ObjectMapper();
	 * Map<String, Object> payloadMap = objectMapper.readValue(jsonPayload,
	 * Map.class);
	 * 
	 * // Print all claims payloadMap.forEach((k, v) -> System.out.println(k + " : "
	 * + v));
	 * 
	 * return token1; }
	 */
